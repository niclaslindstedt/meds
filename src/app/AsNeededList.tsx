        ...
        {entries.map((entry) => (
          ...
          >
            ...
            <div className="mt-2 flex flex-wrap gap-1.5">
              {entry.med.times.length === 0 ? (
                <Chip
                  label={t("asNeeded.logNow")}
                  title={t("asNeeded.logNowLabel", { name: entry.med.name })}
                  onClick={() =>
                    onToggle(
...
function Chip({
  ...
  return (
    <button
      ...
      aria-label={title}
      title={title}
      className="flex min-h-9 items-center gap-1 rounded-full border border-accent/50 px-3 py-1.5 text-xs font-medium text-accent transition-colors hover:bg-accent/10"
    >
