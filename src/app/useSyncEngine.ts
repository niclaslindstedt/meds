...
// union of taps, so two devices that ticked different doses between syncs
// both keep their taps without anyone being asked to choose. The cost is
// that an *un-ticked* dose comes back if the other device still holds it —
// see `docs/sync.md`.
//
...
// store was offered, which is why the picker's options are a reading rather
// than a constant.
...
const CLOUD_FILE_NAME = "meds.json";
...
const ICLOUD_FOLDER = "iCloud Drive/Meds";
...
export function useSyncEngine(
  ...
  // "Demo data" backend has taken over storage, so a quarter of invented taps
    ...
    async (editAtSend: number): Promise<void> => {
      ...
      try {
        ...
        if (err instanceof ConflictError) {
          ...
          // merge is per-item, so neither side's taps are dropped.
  ...
  return {
    ...
    // nothing selected, which would read as "your log is nowhere".
    available:
