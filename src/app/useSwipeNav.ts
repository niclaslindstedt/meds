...
// worse than not having the feature — a control that jumps to the next screen
// instead of moving is a broken control. So the gesture bails at
// `touchstart`, before anything has moved, on:
//
//   - a range input — the whole of a slider is a horizontal drag;
//   - anything inside a dialog — the confirm dialogs live in one;
//   - anything marked `data-swipe-ignore` — the month calendar reads a swipe
//     of its own (see `MonthCalendar.tsx`);
