...
// dose is due are `migrations.ts`, `merge.ts` and `schedule.ts`'s, exactly as
// they are for Dropbox — which is why the host is adapted into
 ...
 *  a revision history, a notion of what a dose is — would be a host holding a
...
export const CLOUD_HOST_EVENT = "meds:cloud-host";
...
export const CLOUD_HOST_PROPERTY = "__medsCloudHost";
 ...
 *  handing anybody's medications to. */
...
function unwrap(result: CloudHostResult): unknown {
  ...
  // `undefined` and, one push later, an empty file where somebody's log
  // were.
