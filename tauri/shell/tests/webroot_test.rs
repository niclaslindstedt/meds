...
use meds_shell::webroot::{content_type_for, percent_decode, resolve_webroot_file, webroot_exists};
...
impl Site {
    fn new(name: &str) -> Self {
        let root = std::env::temp_dir().join(format!("meds-webroot-{name}"));
...
fn a_symlink_out_of_the_webroot_is_refused() {
    ...
    let outside = std::env::temp_dir().join("meds-webroot-symlink-outside.txt");
