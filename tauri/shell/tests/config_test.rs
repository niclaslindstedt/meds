...
//!
//! Both matter more than their size suggests: the origin is what every dose
//! on the machine is keyed to, and the navigation guard is the whole of this
//! shell's security policy.

use meds_shell::config::{app_origin, is_internal_url, start_url, APP_HOST, APP_SCHEME};
...
fn each_platform_gets_the_origin_it_grants() {
    assert_eq!(app_origin(false), "meds://localhost");
    assert_eq!(app_origin(true), "http://meds.localhost");
}

/// Not a tautology: these two words are what every stored dose is keyed to,
...
fn the_origin_is_built_from_the_two_constants() {
    assert_eq!(APP_SCHEME, "meds");
    assert_eq!(APP_HOST, "localhost");
}

#[test]
fn the_window_opens_on_the_entry_page() {
    assert_eq!(start_url("meds://localhost"), "meds://localhost/index.html");
    // A trailing slash on the origin must not become a double one.
    assert_eq!(
        start_url("meds://localhost/"),
        "meds://localhost/index.html"
    );
}

#[test]
fn our_own_pages_navigate_in_the_window() {
    let origin = "meds://localhost";
    assert!(is_internal_url("meds://localhost", origin, None));
    assert!(is_internal_url("meds://localhost/index.html", origin, None));
    assert!(is_internal_url("meds://localhost/privacy/", origin, None));
}
...
fn anything_else_does_not() {
    let origin = "meds://localhost";
    for outside in [
        "https://github.com/niclaslindstedt/meds",
        "https://meds.niclaslindstedt.se/",
        "file:///etc/passwd",
        // The near-miss that a naive `starts_with` on the scheme would admit.
        "meds://localhost.example.com/",
    ] {
        assert!(
...
/// A launch pointed at a remote build (`MEDS_APP_URL`) navigates within THAT
/// site too — otherwise every in-app link would bounce to the browser.
#[test]
fn a_remote_launch_navigates_within_its_own_site() {
    let origin = "meds://localhost";
    let remote = Some("https://meds.niclaslindstedt.se/preview/");
    assert!(is_internal_url(
        "https://meds.niclaslindstedt.se/preview/index.html",
        origin,
        remote
    ));
