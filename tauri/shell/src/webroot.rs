...
//! wrong to simplify.
//!
//! The site is served from a registered private scheme (`meds://`, see
...
//! to lose:
//!
//!  1. **One stable origin.** The user's doses live in IndexedDB and their
//!     settings in `localStorage`, both keyed by origin. `meds://` is a
//!     constant, so the doses survive every update. A `file://` page (or a
//!     server on an ephemeral port) would hand the user a different origin —
//!     and an empty doses — at some point.
//!
...
/// `meds:///C:/…` must not become a drive letter.
