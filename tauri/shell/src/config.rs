...
//! shipping: `MEDS_APP_URL` points the window at a remote URL instead (the
//! `/preview/` deploy slot, say), and `MEDS_WEBROOT` serves a different
...
/// IndexedDB unable to keep a dose between launches. A registered scheme
/// gives one stable origin that the doses are keyed to for the life of the
/// install.
pub const APP_SCHEME: &str = "meds";
...
/// CONSTANT — the origin is what the user's doses are keyed to, so changing
...
pub const WINDOW_TITLE: &str = "Meds";
