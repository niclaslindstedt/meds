...
# Meds — the desktop app

A desktop wrapper around Meds for **Windows, macOS and Linux**. It is a thin
...
minimum. Everything Meds does — the document, the settings, the sync engine —
...
| `src-tauri/src/protocol.rs` | Answering `meds://` off the bundled `webroot/`                |
...
`cargo test -p meds-shell` runs the entire decision layer on a machine with a
...
**The origin is the one thing to be careful with.** The user's doses live in
...
`file://` page is an opaque origin, which would leave the doses unable to
survive a launch.
...
| Variable       | Effect                                                               |
| -------------- | -------------------------------------------------------------------- |
| `MEDS_APP_URL` | Load a remote URL instead of the bundled site (the `/preview/` slot) |
| `MEDS_WEBROOT` | Serve the site from somewhere else without rebuilding                |
...
identifier (`dev.local.meds`). What an installed copy is called and the
