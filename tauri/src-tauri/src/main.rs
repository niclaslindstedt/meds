...
//! The Meds desktop shell — a window showing the bundled site, and nothing
//! else.
//!
...
//! here needed no change to the app at all. Everything Meds does — the
...
use meds_shell::config::APP_SCHEME;
use meds_shell::oauth::{AWAIT_PATH, BEGIN_PATH};
use meds_shell::webroot::webroot_exists;
...
fn main() {
    tauri::Builder::default()
        ...
        // `meds_shell::webroot` for the three properties this arrangement is
        ...
        // `meds_shell::oauth`), which can be minutes, so it answers from a
        ...
        .setup(|app| {
            ...
            if meds_shell::config::remote_app_url().is_none() && !webroot_exists(&root) {
                let message = format!(
                    "Meds could not find the app it is meant to show.\n\n\
                     Looked in: {}\n\n\
        ...
        .expect("the Meds desktop shell could not start");
}
