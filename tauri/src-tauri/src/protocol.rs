...
//! Answering the `meds://` scheme — the effects half of
//! `meds_shell::webroot`, which owns every decision this file acts on.
//!
...
//!
//! `MEDS_WEBROOT` overrides both, which is what lets a build serve a site from
//! somewhere else without recompiling — the same escape hatch `MEDS_APP_URL`
//! gives for a REMOTE site.

use std::fs;
use std::path::PathBuf;

use meds_shell::webroot::{content_type_for, resolve_webroot_file};
...
pub fn webroot_dir(app: &AppHandle) -> PathBuf {
    if let Some(override_dir) = std::env::var_os("MEDS_WEBROOT") {
...
/// returns a body, not a stream, and the largest thing a Meds build contains
