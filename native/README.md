...
The wrapper also decides nothing about the medication log. It moves bytes: a
file in, a file out. When a dose is due, what counts as taken and how two
devices' edits reconcile are the web app's, in `src/app/schedule.ts`,
`stats.ts` and `merge.ts`.
...
One file, `meds.json`, in the app's own iCloud container, under `Documents`
— which is the folder iCloud publishes to the **Files app**, so the reader can
open, copy and delete the file holding their own log. A medication log that
...
the app's sync engine (src/app/useSyncEngine.ts)
   ...
   ▼
window.__medsCloudHost           — installed by src/icloudBridge.ts
   │  postMessage ⇅ injectJavaScript
   ▼
App.tsx → src/icloud.ts → modules/icloud-store
   │
   ▼
iCloud.se.agilator.meds/Documents/meds.json
```
