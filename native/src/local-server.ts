...
// store: every dose the user has logged would appear to vanish. The port is
 ...
 *  the first one is what keeps the user's log across launches.
 *
...
export type LocalServer = {
  /** The origin to point the WebView at, e.g. `http://localhost:8281`. */
  origin: string;
  stop: () => Promise<void>;
};
