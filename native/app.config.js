...
// has no version of its own — it ships one build of the medication log, and the
...
module.exports = () => ({
  expo: {
    ...
    slug: "meds",
    version,
    // A one-column list — today's doses, then the medications — measured for
    // the way a phone is held. Landscape is not broken, but it is not what the
    // layout is for, so the app is pinned.
    ...
    // `se.agilator.meds` in a store build and `dev.local.meds` in a plain
    ...
    ios: {
      ...
      infoPlist: {
        ...
        // folder called "Meds", so the user can see, copy and back up the
        ...
        NSUbiquitousContainers: {
          [ICLOUD_CONTAINER]: {
            ...
            NSUbiquitousContainerName: "Meds",
          },
        },
    ...
    extra: {
      ...
      // debug build at a deployed slot, set EXPO_PUBLIC_MEDS_URL at build
