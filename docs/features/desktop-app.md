...
Meds runs in any browser, and it installs to a home screen or a dock as a
...
[releases page](https://github.com/niclaslindstedt/meds/releases).
...
- **macOS** — the `.dmg`. The app is signed but not notarized, so the first
  ...
  the message about Meds and choose **Open Anyway**. macOS remembers after
  that.
...
around it: the same medications and schedule, the same history, the same settings. Two differences are worth knowing about.
...
the doses you keep in Chrome are not the ones the desktop app opens. Connect the same Dropbox in both (Settings → **Storage**) and they share one log.
