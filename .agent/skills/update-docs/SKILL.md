...
Keeps `docs/` honest (OSS_SPEC §11.1). The docs in this repo describe _behaviour that is derived_, not stored — so a one-line change to a rule in `stats.ts` can silently falsify a paragraph in `docs/schedule.md` without breaking a single test. That is the specific drift this skill exists to catch.
...
4. Pay special attention to numbers and thresholds quoted in prose: the chart and missed-list windows (30 and 14 days), the streak rules, the percentage flooring, the default time slot, the suggestion cap, the debounce interval, the JS budget. These are the values most likely to be edited in code and forgotten in prose.

## Mapping

| Changed source                                 | Doc to update                                                                                           |
| ---------------------------------------------- | ------------------------------------------------------------------------------------------------------- |
| `src/app/schedule.ts`, `src/app/stats.ts`      | `docs/schedule.md`; `docs/features/history.md`; the README's Examples block if the returned shape moved |
| `src/app/catalog.ts`, `src/app/data/*`         | `docs/features/medications.md` (the autocomplete section)                                               |
