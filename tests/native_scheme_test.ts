...
//
// A one-word scheme (`meds://`) can be claimed by any app on the phone, and
...
describe("the URL scheme", () => {
  ...
  it("is the development bundle id in a plain checkout", () => {
    ...
    expect(expo.scheme).toBe("dev.local.meds");
    expect(expo.scheme).toBe(expo.ios?.bundleIdentifier);
  });

  it("follows APP_BUNDLE_ID in a store build", () => {
    const { expo } = loadConfig({ APP_BUNDLE_ID: "se.agilator.meds" });
    expect(expo.scheme).toBe("se.agilator.meds");
    expect(expo.scheme).toBe(expo.ios?.bundleIdentifier);
  });
