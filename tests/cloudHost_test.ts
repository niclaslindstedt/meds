...
// host is refused rather than handed somebody's medications.
...
describe("parseHostEntries", () => {
  it("keeps a path with its revision, and a path without one", () => {
    expect(
      parseHostEntries([
        { path: "meds.json", rev: "1700000000000000:412" },
        { path: "other.json" },
      ]),
    ).toEqual([
      { path: "meds.json", rev: "1700000000000000:412" },
  ...
  it("drops one malformed entry rather than the whole listing", () => {
    ...
    expect(
      parseHostEntries([
        ...
        { path: "" },
        { rev: "x" },
        { path: "meds.json", rev: 7 },
        "meds.json",
      ]),
    ).toEqual([{ path: "meds.json" }]);
  });

  it("is empty for anything that is not a list", () => {
    expect(parseHostEntries(undefined)).toEqual([]);
    expect(parseHostEntries({ path: "meds.json" })).toEqual([]);
  });
});
...
describe("the strings a host has to match", () => {
  it("pins the property and the event", () => {
    ...
    expect(CLOUD_HOST_PROPERTY).toBe("__medsCloudHost");
    expect(CLOUD_HOST_EVENT).toBe("meds:cloud-host");
...
describe("createCloudHostAdapter", () => {
    ...
    const adapter = createCloudHostAdapter(
      {
        ...
        list: () =>
          Promise.resolve({
            ok: true,
            value: [{ path: "meds.json", rev: "17:412" }],
          }),
        read: (path: string) => {
          asked = path;
      ...
      { label: "iCloud Drive", fileName: "meds.json" },
    );
    await expect(adapter.load()).resolves.toEqual({
      text: '{"version":2}',
      revision: "17:412",
    });
    expect(asked).toBe("meds.json");
  });
  ...
  it("refuses an answer it cannot read, rather than passing it on", async () => {
    ...
    // later, an empty file where somebody's log was.
